...
RC Table::drop(const char* dir) {
  ...
  const int index_num = table_meta_.index_num();
    for (int i = 0; i < index_num; i++) {  // 清理所有的索引相关文件数据与索引元数据
      BplusTreeIndex* tmp = new BplusTreeIndex();
        tmp->transform(table_meta_.index(i),table_meta_.field(i));
        tmp->close();//table_meta_.index(i)返回一个IndexMeta *，希望能转换成BplusTreeIndex*
